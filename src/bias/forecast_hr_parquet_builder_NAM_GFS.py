...
def make_dirs(year, month, day, fh, model):
    if not os.path.exists(f"/home/aevans/ai2es/lstm/{model.upper()}/fh_{fh}/"):
...
def main(start_date, end_date, fh, model):
    ...
    while start_date <= end_date:
        ...
        the_df.to_parquet(
            f"/home/aevans/ai2es/lstm/{model.upper()}/fh_{fh}/{year}/{month}/{year}{month}{day}_{model.lower()}_fh{fh}.parquet"
        )
        print(the_df)
