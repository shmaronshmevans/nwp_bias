...
def main(month, year, model, init, mask_water=True):
    ...
    if mask_water:
        df_model_nysm_sites.to_parquet(
            f"{savedir}ny/{model}_{init}z_{month}-{year}_direct_compare_to_nysm_sites_mask_water.parquet"
        )
    else:
        df_model_nysm_sites.to_parquet(
            f"{savedir}ny/{model}_{init}z_{month}-{year}_direct_compare_to_nysm_sites.parquet"
        )
...
#         results = [pool.apply(main, args=(str(month).zfill(2), year, model, "12")) for year in ranger]
