...
final_df[f"trial --{num_hidden_units}_{learning_rate}"] = 00
...
for feature in features:
    print(feature)
    new_features = []
    for x, _ in enumerate(features):
        if features[x] == feature:
            continue
        else:
            new_features.append(features[x])
...
for x, _ in enumerate(final_df["feature"]):
    keycard = features.index(final_df["feature"].iloc[x])
    loss_val = loss_ls[keycard]
    final_df[f"trial --{num_hidden_units}_{learning_rate}"].iloc[x] = loss_val

final_df.to_csv(
    f"/home/aevans/nwp_bias/src/machine_learning/data/lstm_feature_weights_western_plateau_{learning_rate}_{num_hidden_units}.csv"
)
