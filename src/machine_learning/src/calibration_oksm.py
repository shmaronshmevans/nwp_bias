...
def refit(df, model):
    ...
    for i in indexes:
        if model != "bnn":
            target, lstm_val, _ = df.loc[i].values
            targets.append(target)
            lstms.append(lstm_val)
        else:
            target, lstm_val, _, _, _ = df.loc[i].values
            targets.append(target)
            lstms.append(lstm_val)
...
def outlier_fit(df, hrrr_var, model, maxy=3):
    ...
    for i in top_200_indexes:
        if model != "bnn":
            ...
            (target, lstm_val, _, _, _) = df_.loc[i].values
...
def calibration(og_df, model_df, hrrr_var, model):
    ...
    if slope > 5:
        ...
        try:
            ...
            df_out, multiply = outlier_fit(df1, hrrr_var, model)
            print(multiply)
        except:
            df_out, multiply = outlier_fit(df1, hrrr_var, model, maxy=75)
    ...
    for s in stations:
        ...
        for fh in np.arange(1, 19):
            try:
                ...
                df = calibration(og_df, df, var, model_type)
