...
class BlockDiagonal(nn.Module):
    def __init__(self, in_features, out_features, num_blocks, device):
        ...
        print("infeatures", self.in_features)
        print("blocks", self.num_blocks)
...
class sLSTMBlock(nn.Module):
    def __init__(self, input_size, hidden_size, num_heads, device, proj_factor=4 / 3):
        ...
        print("hidden", self.hidden_size, "heads", self.num_heads)
        ...
        self.Wz = BlockDiagonal(input_size, hidden_size, num_heads, device)
        self.Wi = BlockDiagonal(input_size, hidden_size, num_heads, device)
        self.Wf = BlockDiagonal(input_size, hidden_size, num_heads, device)
        self.Wo = BlockDiagonal(input_size, hidden_size, num_heads, device)

        self.Rz = BlockDiagonal(hidden_size, hidden_size, num_heads, device)
        self.Ri = BlockDiagonal(hidden_size, hidden_size, num_heads, device)
        self.Rf = BlockDiagonal(hidden_size, hidden_size, num_heads, device)
        self.Ro = BlockDiagonal(hidden_size, hidden_size, num_heads, device)
    ...
    def forward(self, x, prev_state):
        x = x.to(self.device)
        h_prev, c_prev, n_prev, m_prev = [s.to(self.device) for s in prev_state]
...
class sLSTM(nn.Module):
    ...
    def __init__(
        ...
        self.layers = nn.ModuleList(
            [
                sLSTMBlock(input_size, hidden_size, num_heads, device, proj_factor)
...
class mLSTMBlock(nn.Module):
    def __init__(self, input_size, hidden_size, num_heads, device, proj_factor=2):
        ...
        self.Wq = BlockDiagonal(
            int(input_size * proj_factor), hidden_size, num_heads, device
        )
...
class mLSTM(nn.Module):
    ...
    def __init__(
        ...
        self.layers = nn.ModuleList(
            [
                mLSTMBlock(input_size, hidden_size, num_heads, device, proj_factor)
                for _ in range(num_layers)
            ]
        )
...
class xLSTM(nn.Module):
    ...
    def __init__(
        ...
        for layer_type in layers:
            if layer_type == "s":
                layer = sLSTMBlock(
                    input_size, hidden_size, num_heads, device, proj_factor_slstm
                )
            elif layer_type == "m":
                layer = mLSTMBlock(
                    input_size, hidden_size, num_heads, device, proj_factor_mlstm
                )
            else:
                raise ValueError(
            ...
            self.linear = nn.Linear(self.input_size, self.forecast_hour)
