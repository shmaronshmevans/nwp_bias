...
def train_model(epoch, device, mask, decision, train_loader, optimizer, log_interval, batch_size):
    print("Hello World!")
    print(device)
    mask.train()
    decision.train()
...
def test_model(epoch, device, mask, decision, test_loader, log_interval, batch_size):
    ...
    for batch_idx, (data, target) in enumerate(test_loader):
        ...
        if batch_idx % log_interval == 0:
            print('Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.6f}'.format(epoch, batch_idx*len(data), len(test_loader.dataset), 100.*batch_idx/len(test_loader), loss.item()))
            test_loss.append(loss.item())
            test_counter.append((batch_idx*batch_size) + ((epoch-1)*len(test_loader.dataset)))
...
def main(station, fh, sequence_length, batch_size, epochs, log_interval):
    ...
    for epoch in range(1, epochs+1):
        print(epoch)
        train_ls = train_model(epoch, device, mask, decision, train_loader, optimizer, log_interval, batch_size)
        train_loss.append(train_ls)
        test_ls = test_model(epoch, device, mask, decision, test_loader, log_interval, batch_size)
...
main(station='WANT', fh=4, sequence_length=120, batch_size=int(20e2), epochs=5, log_interval=10)
